...
export class SimpleChartComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit(): void {
    ...
    if (this.baseData.length) {
      this.pushCandlesToSeries(this.baseData, true);
    }
  }
  ...
  private loadSymbolsAndCandles(): void {
    this.marketService
      ...
      .subscribe({
        next: () => {
          this.loading = false;
          if (this.chartReady && this.baseData.length) {
            this.pushCandlesToSeries(this.baseData, true);
          }
  ...
  private pushCandlesToSeries(candles: InternalCandle[], fitContent: boolean): void {
    if (!this.candleSeries) return;
    this.candleSeries.setData(internalArrayToLw(candles));
    if (fitContent) {
  ...
  private onLiveKlineUpdate(liveUpdate: {
    ...
    this.ngZone.runOutsideAngular(() => {
      ...
      this.candleSeries?.update(internalToLwCandle(last));
      this.ngZone.run(() => {
        this.updatePriceBadgePosition();
        this.cdr.markForCheck();
      });
  ...
  private applyInternalLiveCandle(liveCandle: InternalCandle): void {
    ...
    this.candleSeries?.update(internalToLwCandle(liveCandle));
    this.ngZone.run(() => {
      this.updatePriceBadgePosition();
      this.cdr.markForCheck();
    });
