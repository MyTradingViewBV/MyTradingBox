...
export class CoinInfoComponent implements OnChanges {
  ...
  private loadInfo(symbol: string): void {
    ...
    this.http.get<any>(`https://api.binance.com/api/v3/ticker/24hr?symbol=${upper}`).subscribe({
      next: (t) => {
        partial.priceUsd = parseFloat(t.lastPrice);
        partial.change24hPct = parseFloat(t.priceChangePercent);
        partial.high24h = parseFloat(t.highPrice);
        partial.low24h = parseFloat(t.lowPrice);
        partial.volume24h = parseFloat(t.quoteVolume);
        this.info = { ...partial };
        this.cdr.markForCheck();
      },
      error: () => {},
    ...
    // CoinGecko search → coin detail (free, no key)
    const base = this.extractBase(upper);
    this.http.get<any>(`https://api.coingecko.com/api/v3/search?query=${base}`).subscribe({
      next: (resp) => {
        const coin = (resp?.coins ?? []).find((c: any) =>
        ...
        if (!coin?.id) { this.loading = false; this.cdr.markForCheck(); return; }
